    ...
    while state.r_draw.dc_x <= x2 {
        if *state.r_segs.maskedtexturecol.offset(state.r_draw.dc_x as isize) as i32 != SHRT_MAX {
            ...
            state.r_draw.dc_iscale = (0xffffffff as u32)
                ...
                as fixed_t;
            col = R_GetColumn(state, texnum, *state.r_segs.maskedtexturecol.offset(state.r_draw.dc_x as isize) as i32)
...
pub unsafe fn R_RenderSegLoop(state: &mut GameState) {
    ...
    while state.r_segs.rw_x < state.r_segs.rw_stopx {
        ...
        if state.r_segs.segtextured {
            angle = state.r_segs.rw_centerangle.wrapping_add(state.r_main.xtoviewangle[state.r_segs.rw_x as usize]) >> ANGLETOFINESHIFT;
            texturecolumn = state.r_segs.rw_offset - FixedMul(finetangent[angle as usize], state.r_segs.rw_distance);
            ...
            state.r_draw.dc_iscale = (0xffffffff as u32).wrapping_div(state.r_segs.rw_scale as u32) as fixed_t;
        } else {
        ...
        if state.r_segs.midtexture != 0 {
            ...
            state.r_draw.dc_source = R_GetColumn(state, state.r_segs.midtexture, texturecolumn as i32);
            ...
            if state.r_segs.toptexture != 0 {
                ...
                if mid >= yl {
                    ...
                    state.r_draw.dc_source = R_GetColumn(state, state.r_segs.toptexture, texturecolumn as i32);
                    state.r_main.colfunc.expect("non-null function pointer")(state);
            ...
            if state.r_segs.bottomtexture != 0 {
                ...
                if mid <= yh {
                    ...
                    state.r_draw.dc_source = R_GetColumn(state, state.r_segs.bottomtexture, texturecolumn as i32);
                    state.r_main.colfunc.expect("non-null function pointer")(state);
            ...
            if state.r_segs.maskedtexture {
                *state.r_segs.maskedtexturecol.offset(state.r_segs.rw_x as isize) = texturecolumn as i16;
            }
        }
...
pub unsafe fn R_StoreWallRange(state: &mut GameState, mut start: i32, mut stop: i32) {
    ...
    let mut lightnum: i32 = 0;
    if state.r_bsp.ds_p == (&raw mut state.r_bsp.drawsegs as *mut drawseg_t).offset(MAXDRAWSEGS as isize) as *mut drawseg_t
    {
        return;
    }
    ...
    offsetangle = (state.r_segs.rw_normalangle.wrapping_sub(state.r_segs.rw_angle1 as angle_t) as i32).abs() as angle_t;
    if offsetangle > ANG90 as angle_t {
        offsetangle = ANG90 as angle_t;
    }
    distangle = (ANG90 as angle_t).wrapping_sub(offsetangle);
    let (v1x, v1y) = ((*(*state.r_bsp.curline).v1).x, (*(*state.r_bsp.curline).v1).y);
    ...
    let angle1 = state.r_main.viewangle.wrapping_add(state.r_main.xtoviewangle[start as usize]);
    ...
    if stop > start {
        (*state.r_bsp.ds_p).scale2 =
            {
                let angle2 = state.r_main.viewangle.wrapping_add(state.r_main.xtoviewangle[stop as usize]);
                R_ScaleFromGlobalAngle(state, angle2)
            };
        state.r_segs.rw_scalestep = (((*state.r_bsp.ds_p).scale2 as i32 - state.r_segs.rw_scale as i32) / (stop - start)) as fixed_t;
        (*state.r_bsp.ds_p).scalestep = state.r_segs.rw_scalestep;
    } else {
        (*state.r_bsp.ds_p).scale2 = (*state.r_bsp.ds_p).scale1;
    }
    state.r_segs.worldtop = ((*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).ceilingheight - state.r_main.viewz) as i32;
    state.r_segs.worldbottom = ((*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).floorheight - state.r_main.viewz) as i32;
    ...
    if state.r_bsp.backsector.is_none() {
        state.r_segs.midtexture = *state.r_data.texturetranslation.offset((*state.p_setup.side_mut(state.r_bsp.sidedef)).midtexture as isize);
        ...
        if (*state.r_bsp.linedef).flags as i32 & ML_DONTPEGBOTTOM != 0 {
            vtop =
                (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).floorheight + *state.r_data.textureheight.offset((*state.p_setup.side_mut(state.r_bsp.sidedef)).midtexture as isize);
        ...
        if (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).floorheight > (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).floorheight {
            (*state.r_bsp.ds_p).silhouette = SIL_BOTTOM;
            (*state.r_bsp.ds_p).bsilheight = (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).floorheight;
        } else if (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).floorheight > state.r_main.viewz {
            (*state.r_bsp.ds_p).silhouette = SIL_BOTTOM;
            (*state.r_bsp.ds_p).bsilheight = INT_MAX as fixed_t;
        }
        if (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).ceilingheight < (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).ceilingheight {
            (*state.r_bsp.ds_p).silhouette |= SIL_TOP;
            (*state.r_bsp.ds_p).tsilheight = (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).ceilingheight;
        } else if (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).ceilingheight < state.r_main.viewz {
            (*state.r_bsp.ds_p).silhouette |= SIL_TOP;
            (*state.r_bsp.ds_p).tsilheight = INT_MIN as fixed_t;
        }
        if (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).ceilingheight <= (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).floorheight {
            (*state.r_bsp.ds_p).sprbottomclip =
                &raw mut state.r_things.negonearray as *mut i16;
            (*state.r_bsp.ds_p).bsilheight = INT_MAX as fixed_t;
            (*state.r_bsp.ds_p).silhouette |= SIL_BOTTOM;
        }
        if (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).floorheight >= (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).ceilingheight {
            (*state.r_bsp.ds_p).sprtopclip =
                &raw mut state.r_things.screenheightarray as *mut i16;
            (*state.r_bsp.ds_p).tsilheight = INT_MIN as fixed_t;
            (*state.r_bsp.ds_p).silhouette |= SIL_TOP;
        }
        state.r_segs.worldhigh = ((*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).ceilingheight - state.r_main.viewz) as i32;
        state.r_segs.worldlow = ((*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).floorheight - state.r_main.viewz) as i32;
        if (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).ceilingpic as i32 == state.r_sky.skyflatnum
            && (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).ceilingpic as i32 == state.r_sky.skyflatnum
        {
        ...
        if state.r_segs.worldhigh != state.r_segs.worldtop
            || (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).ceilingpic as i32 != (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).ceilingpic as i32
            || (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).lightlevel as i32 != (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).lightlevel as i32
        {
            state.r_segs.markceiling = true;
        } else {
        ...
        if state.r_segs.worldhigh < state.r_segs.worldtop {
            state.r_segs.toptexture = *state.r_data.texturetranslation.offset((*state.p_setup.side_mut(state.r_bsp.sidedef)).toptexture as isize);
            if (*state.r_bsp.linedef).flags as i32 & ML_DONTPEGTOP != 0 {
                ...
                vtop = (*state.p_setup.sector_mut(state.r_bsp.backsector.unwrap())).ceilingheight
                    + *state.r_data.textureheight.offset((*state.p_setup.side_mut(state.r_bsp.sidedef)).toptexture as isize);
                state.r_segs.rw_toptexturemid = vtop - state.r_main.viewz;
            }
        }
        if state.r_segs.worldlow > state.r_segs.worldbottom {
            state.r_segs.bottomtexture = *state.r_data.texturetranslation.offset((*state.p_setup.side_mut(state.r_bsp.sidedef)).bottomtexture as isize);
            if (*state.r_bsp.linedef).flags as i32 & ML_DONTPEGBOTTOM != 0 {
        ...
        if (*state.p_setup.side_mut(state.r_bsp.sidedef)).midtexture != 0 {
            ...
            state.r_segs.maskedtexturecol = state.r_plane.lastopening.offset(-(state.r_segs.rw_x as isize));
            (*state.r_bsp.ds_p).maskedtexturecol = state.r_segs.maskedtexturecol;
            state.r_plane.lastopening = state.r_plane.lastopening.offset((state.r_segs.rw_stopx - state.r_segs.rw_x) as isize);
        }
    }
    state.r_segs.segtextured = (state.r_segs.midtexture | state.r_segs.toptexture | state.r_segs.bottomtexture) != 0 || state.r_segs.maskedtexture;
    if state.r_segs.segtextured {
        offsetangle = state.r_segs.rw_normalangle.wrapping_sub(state.r_segs.rw_angle1 as angle_t);
        ...
        if state.r_segs.rw_normalangle.wrapping_sub(state.r_segs.rw_angle1 as angle_t) < ANG180 {
            state.r_segs.rw_offset = -state.r_segs.rw_offset;
        }
        state.r_segs.rw_offset += (*state.p_setup.side_mut(state.r_bsp.sidedef)).textureoffset + (*state.r_bsp.curline).offset;
    ...
    if (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).floorheight >= state.r_main.viewz {
        state.r_segs.markfloor = false;
    }
    if (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).ceilingheight <= state.r_main.viewz
        && (*state.p_setup.sector_mut(state.r_bsp.frontsector.unwrap())).ceilingpic as i32 != state.r_sky.skyflatnum
    {
        state.r_segs.markceiling = false;
    }
    ...
    state.r_segs.topfrac = (state.r_main.centeryfrac >> 4 as i32) - FixedMul(state.r_segs.worldtop as fixed_t, state.r_segs.rw_scale);
    state.r_segs.bottomstep = -FixedMul(state.r_segs.rw_scalestep, state.r_segs.worldbottom as fixed_t);
    state.r_segs.bottomfrac = (state.r_main.centeryfrac >> 4 as i32) - FixedMul(state.r_segs.worldbottom as fixed_t, state.r_segs.rw_scale);
    if !state.r_bsp.backsector.is_none() {
        state.r_segs.worldhigh >>= 4 as i32;
        state.r_segs.worldlow >>= 4 as i32;
        if state.r_segs.worldhigh < state.r_segs.worldtop {
            state.r_segs.pixhigh = (state.r_main.centeryfrac >> 4 as i32) - FixedMul(state.r_segs.worldhigh as fixed_t, state.r_segs.rw_scale);
            state.r_segs.pixhighstep = -FixedMul(state.r_segs.rw_scalestep, state.r_segs.worldhigh as fixed_t);
        }
        if state.r_segs.worldlow > state.r_segs.worldbottom {
            state.r_segs.pixlow = (state.r_main.centeryfrac >> 4 as i32) - FixedMul(state.r_segs.worldlow as fixed_t, state.r_segs.rw_scale);
            state.r_segs.pixlowstep = -FixedMul(state.r_segs.rw_scalestep, state.r_segs.worldlow as fixed_t);
        }
    }
    if state.r_segs.markceiling {
        let (ceilingplane, rw_x, rw_stopx_1) = (state.r_plane.ceilingplane, state.r_segs.rw_x, state.r_segs.rw_stopx - 1 as i32);
        state.r_plane.ceilingplane = R_CheckPlane(state, ceilingplane, rw_x, rw_stopx_1);
    }
    if state.r_segs.markfloor {
        let (floorplane, rw_x2, rw_stopx_2) = (state.r_plane.floorplane, state.r_segs.rw_x, state.r_segs.rw_stopx - 1 as i32);
        state.r_plane.floorplane = R_CheckPlane(state, floorplane, rw_x2, rw_stopx_2);
    }
    R_RenderSegLoop(state);
    if ((*state.r_bsp.ds_p).silhouette & SIL_TOP != 0 || state.r_segs.maskedtexture) && (*state.r_bsp.ds_p).sprtopclip.is_null() {
        memcpy(
            state.r_plane.lastopening as *mut ::core::ffi::c_void,
            (&raw mut state.r_plane.ceilingclip as *mut i16).offset(start as isize) as *const ::core::ffi::c_void,
        ...
        state.r_plane.lastopening = state.r_plane.lastopening.offset((state.r_segs.rw_stopx - start) as isize);
    }
    if ((*state.r_bsp.ds_p).silhouette & SIL_BOTTOM != 0 || state.r_segs.maskedtexture) && (*state.r_bsp.ds_p).sprbottomclip.is_null() {
        memcpy(
            state.r_plane.lastopening as *mut ::core::ffi::c_void,
            (&raw mut state.r_plane.floorclip as *mut i16).offset(start as isize) as *const ::core::ffi::c_void,
            (2 as i32 * (state.r_segs.rw_stopx - start)) as size_t,
        );
